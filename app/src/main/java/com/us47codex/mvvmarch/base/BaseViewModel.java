            ...
            try {
                ...
                if (!AppUtils.isEmpty(res)) {
                    ...
                    if (obj.has(Constant.KEY_CODE)) {
                        AppLog.loge(TAG, "parseOnSuccess :: API TAG : " + API_TAG + ":: Error : " + obj.getString(Constant.KEY_CODE));
                        if (showProgressBar)
...
//                        responseNavigator.handleSuccess(obj);
                    ...
                    AppLog.loge(TAG, "parseOnSuccess :: API TAG : " + API_TAG + ":: Error : Response Null");
                    if (showProgressBar)
                        getStatusBehaviorRelay().accept(ApiCallStatus.ERROR);
                ...
                e.printStackTrace();
                AppLog.loge(TAG, "parseOnSuccess :: API TAG : " + API_TAG + ":: Error : " + e);
                if (showProgressBar)
                    getStatusBehaviorRelay().accept(ApiCallStatus.ERROR);
            ...
            if (/*response.code() == Constant.BAD_REQUEST ||*/
                ...
                getCompositeDisposable().add(
                        AppUtils.getErrorMessageFRomErrorCode(response.code())
                                ...
                                .doOnSuccess(message -> {
                                    AppLog.loge(TAG, "parseOnSuccess :: API TAG : " + API_TAG + ":: Error : " + message);
                                    if (showProgressBar)
                                        getStatusBehaviorRelay().accept(ApiCallStatus.ERROR);
    ...
    protected void parseOnError(Throwable error, String API_TAG, boolean showProgressBar) {
        try {
            if (error.getCause() == null) {
                ...
                AppLog.loge(TAG, "parseOnError :: API TAG : " + API_TAG + ":: Error : " + error.getLocalizedMessage());
                if (showProgressBar)
                    getStatusBehaviorRelay().accept(ApiCallStatus.ERROR);
